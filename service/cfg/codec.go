...
func IsJson(content string) bool {
	...
	err := jsoniter.Unmarshal([]byte(content), &object)
	if err != nil {
		return false
	}
	return true
...
func JsonToMap(contentOfJson string) (map[string]interface{}, error) {
	...
	err := jsoniter.Unmarshal([]byte(contentOfJson), &resultMap)
	if err != nil {
...
func JsonToKvList(content string) ([]StringPair, error) {
	y, err := JsonToYaml(content)
	if err != nil {
		return nil, err
	}
	return YamlToKvList(y)
}

func JsonToFlatMap(content string) (map[string]string, error) {
	kvs, err := JsonToKvList(content)
	if err != nil {
		return nil, err
	}
	result := make(map[string]string, len(kvs))
...
func JsonToYaml(contentOfJson string) (string, error) {
	...
	var object interface{}
	err := jsoniter.Unmarshal([]byte(contentOfJsonTem), &object)
	if err != nil {
		return "", err
	}
